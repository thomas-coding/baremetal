/*
 * Contains the functions support console printf
 *
...
static void padding(const int32_t l_flag, struct params_s *par)
{
	uint32_t i = 0;
...
static int32_t get_num(char **linep)
{
	int32_t n = 0;
	char *cp = NULL;
 ...
 try_next:
		...
		case 'o':
			if (long_flag || ch == 'O') {
				...
				out_num((int32_t)va_arg(argp, int32_t), 8L, &par);
				continue;
			}
		case 'd':
			if (long_flag || ch == 'D') {
				...
				out_num((int32_t)va_arg(argp, int32_t), 10L, &par);
				continue;
			}
		case 'u':
			if (long_flag || ch == 'U') {
				...
				out_num((uint32_t)va_arg(argp, uint32_t), 10L, &par);
				continue;
			}
		case 'x':
			if (ch == 'X')
			...
			if (long_flag) {
				...
				out_num((uint32_t)va_arg(argp, uint32_t), 16L, &par);
				continue;
			}
		case 's':
